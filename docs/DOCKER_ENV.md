...
| `LOG_EVENT_CHUNK_SIZE` | `log-event-chunk-size` | `999`   | How many log IDs bob requests per RequestLog packet. Default fits a full tick's worth of logs in one round-trip. Lower (e.g. 512 or 1024) on links where 4 MB responses cause TCP-level queueing or BM-side stalls; raise to test BMs that accept larger requests. Must be > 0. |
